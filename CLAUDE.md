...
├── config.lua    -- Configuration management, provider flattening/merging
...
├── provider.lua  -- HTTP client for LLM APIs (OpenAI/OpenRouter/DeepL), request building
...
**Provider Configuration Hierarchy**: The `config.lua` module flattens provider configs from `provider@model` format. Default providers are merged with user-provided configs via `install_providers()`. Each provider entry contains `base_url`, `api_key_name`, `stream`, `multi_round`, and model-specific options.
...
- `llm.provider.ProviderOptions` - Provider configuration
...
| `:LLM ChatProviders` | `api.open_chat_provider_selector()` |
